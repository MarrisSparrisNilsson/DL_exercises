# Taken from https://github.com/williamlidberg/Mapping-drainage-ditches-in-forested-landscapes-using-deep-learning-and-aerial-laser-scanning/blob/DitchNet-1-m-resolution/utils/unet.py

# implementation adapted from:
# https://keras.io/examples/vision/oxford_pets_image_segmentation/

from crfrnn_layer import (
    CrfRnnLayer,  # don't forget to add this to path: export PYTHONPATH=/home/william/Downloads/crfasrnn_keras-master/src/:$PYTHONPATH
)
...
class XceptionUNet(object):
    ...
    def __setup_model(self):
        ...
        #                        padding="same")(x)
        outputs = layers.Conv2D(self.classes, 3, activation=self.activation, padding="same")(x)
...
class XceptionUNetCRF(XceptionUNet):
    ...
    def __setup_crf_model(self):
        ...
        outputs = layers.Reshape((self.input_shape[0], self.input_shape[1], self.classes))(self.model.outputs[0])
        # create fake RGB image
        inputs = layers.concatenate([self.model.inputs[0], self.model.inputs[0], self.model.inputs[0]], axis=3)
        crf_layer = CrfRnnLayer(
        ...
        model = keras.Model(inputs=self.model.input, outputs=outputs)
        return model
...
class UNet(object):
    ...
    def __setup_model(self):
        ...
        for i, size in enumerate(self.down_sample):
            conv1 = layers.Conv2D(size, 3, activation="relu", padding="same", kernel_initializer="he_normal")(x)
            conv1 = layers.Conv2D(size, 3, activation="relu", padding="same", kernel_initializer="he_normal")(conv1)
        ...
        conv5 = layers.Conv2D(size, 3, activation="relu", padding="same", kernel_initializer="he_normal")(x)
        conv5 = layers.Conv2D(size, 3, activation="relu", padding="same", kernel_initializer="he_normal")(conv5)
        x = layers.Dropout(0.5)(conv5)

        # Up sampling
        for i, size in enumerate(self.up_sample):
            up6 = layers.Conv2D(size, 2, activation="relu", padding="same", kernel_initializer="he_normal")(
                layers.UpSampling2D(size=(2, 2))(x)
            )

            merge6 = layers.concatenate([conv_layers.pop(), up6], axis=3)
            conv6 = layers.Conv2D(size, 3, activation="relu", padding="same", kernel_initializer="he_normal")(merge6)
            x = layers.Conv2D(size, 3, activation="relu", padding="same", kernel_initializer="he_normal")(conv6)

        conv9 = layers.Conv2D(2, 3, activation="relu", padding="same", kernel_initializer="he_normal")(x)
