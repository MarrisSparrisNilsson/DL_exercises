# Taken from https://github.com/williamlidberg/Mapping-drainage-ditches-in-forested-landscapes-using-deep-learning-and-aerial-laser-scanning/blob/DitchNet-1-m-resolution/utils/generator.py
...
class DataGenerator(tf.keras.utils.Sequence):
    def __init__(
        ...
        assert (
            (size is None and include is None and exclude is not None)
        ...
        self.selected = self.__select_imgs(self.paths, size, include, exclude, self.rng)
    ...
    def __read_paths(self, img_path, gt_path):
        paths = []
        imgs = [
            os.path.join(img_path, f) for f in os.listdir(img_path) if not f.startswith("._") and f.endswith(".tif")
        ]
        imgs = sorted(imgs)
        gts = [os.path.join(gt_path, f) for f in os.listdir(gt_path) if not f.startswith("._") and f.endswith(".tif")]
        gts = sorted(gts)
    ...
    def __get_data(self, batch):
        ...
        for img_path, gt_path in batch:
            ...
            if self.augment:
                ...
                if select[0]:
                    img, gt = self.create_rotation_augmentation(img, gt, self.rng)
                if select[1]:
            ...
            gt_new = np.zeros(2 * gt.shape[0] * gt.shape[1]).reshape((*gt.shape, 2))
            # non-ditch image
            gt_new[:, :, 0] = 1 - gt
            # ditch image
            gt_new[:, :, 1] = gt
    ...
    def __warp_imgs(self, img, gt, transform):
        ...
        warped_gt = cv2.warpAffine(gt, transform, dsize=(x, y), borderValue=borderValue)
        warped_gt = np.round(warped_gt)

        return warped_img, warped_gt

    def create_affine_transform_augmentation(self, img, gt, rng, random_limits=(0.8, 1.1)):
        """
        ...
        random_shift = (rng.random(6).reshape((3, 2)) - 0.5) * 2 * (random_limits[1] - random_limits[0]) / 2 + np.mean(
            random_limits
        )
